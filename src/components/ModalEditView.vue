<template>
        ...
        <div class="modal-content">
          <div class="modal-header">
            <div class="modal-title-wrapper">
              ...
              <h3 class="modal-title">Редактировать событие</h3>
            </div>
          </div>
          ...
          <form class="modal-form">
            <div class="input-group">
              ...
              >
              <span v-if="isTitleEmpty" class="error-message">{{ error }}</span>
            </div>
            <div class="input-group">
              ...
              <input
                ...
                required
              >
              <span v-if="isTimeEmpty" class="error-message">Выберите время события</span>
            </div>
            <div class="input-group">
              ...
              ></textarea>
              <span v-if="isDescriptionEmpty" class="error-message">{{ error }}</span>
            </div>
          </form>
          <div class="modal-footer">
            <button class="cancel-btn" @click="$emit('close')">Отмена</button>
            <button class="submit-btn" @click="saveChanges">Сохранить</button>
          </div>
        </div>
      </div>
    </div>
  ...
  export default {
    ...
    created() {
      ...
      this.eventDescription = this.event.desc
      this.eventTime = this.event.time
      this.uuid = this.event.uuid
    },
    methods: {
      saveChanges() {
        ...
        if (!this.eventDescription?.trim()) {
          this.error = 'Описание событ��я не может быть пустым'
